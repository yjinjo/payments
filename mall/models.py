...
class AbstractPortonePayment(models.Model):
    ...
    meta = models.JSONField("포트원 결제 내역", default=dict, editable=False)
    uid = models.UUIDField("쇼핑몰 결제 식별자", default=uuid4, editable=False)
    ...
    pay_method = models.CharField(
        "결제 수단", max_length=20, choices=PayMethod.choices, default=PayMethod.CARD
    )
    pay_status = models.CharField(
        "결제 상태", max_length=20, choices=PayStatus.choices, default=PayStatus.READY
    )
    ...
    def update(self):
        ...
        # TODO: 결제는 되었는데, 결제금액이 맞지 않는 경우, -> 의심된다 플래그를 지정한다든지.
